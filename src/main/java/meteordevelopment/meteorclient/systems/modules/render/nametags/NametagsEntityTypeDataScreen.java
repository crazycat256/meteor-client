...
public class NametagsEntityTypeDataScreen extends WindowScreen {
    ...
    public void initWidgets() {
        ...
        sgGeneral.add(new BoolSetting.Builder()
            .name("only-visible")
            .description("Only renders nametags for players that are visible.")
            .defaultValue(false)
            .onModuleActivated(settingBooleanSetting -> settingBooleanSetting.set(entityTypeData.onlyVisible))
            .onChanged(settingBoolean -> {
                entityTypeData.onlyVisible = settingBoolean;
                changed(entityTypeData, entityType, setting);
            })
            .build()
        );
